...
const char* ssid = "Suze"; //SSID da rede WiFi
const char* password = "manurafa"; //senha da rede WiFi
...
String topicoPrefixo = "MACK32027494"; //para o prefixo do topico, utilizar MACK seguido do TIA
...
void loop() {
  ...
  if (isnan(t) || isnan(h)) {
   ...
   if (t > 10){
    Serial.println("A estufa está com a temperatura abaixo do ideal! A led foi acionada para seu aquecimento!");
    digitalWrite(D7, HIGH);
    delay(2000);
    mensagem_1 = "A estufa está com a temperatura abaixo do ideal! A led foi acionada para seu aquecimento!";
  }
  else if(t > 20){
    Serial.println("A estufa está com a temperatura acima do ideal!");
    digitalWrite(D7, LOW);
    delay(2000);
    mensagem_2 = "A estufa está com a temperatura acima do ideal!";
  }
   //Publica mensagem
  long now = millis();
  if (now - lastMsg > 2000) {
